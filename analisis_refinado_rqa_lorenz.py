#!/usr/bin/env python3
"""
analisis_refinado_rqa_lorenz.py

Pipeline REFINADO + RQA aplicado al **atractor de Lorenz** (sintético controlado).

Usa las tres variables (x, y, z) como las tres "componentes/regiones".

Incluye inyección de cambios de régimen modificando el parámetro rho en momentos específicos.

Objetivo: probar si los picos "hiperestructurales" (refinado + RQA Config B)
anticipan cambios en la estructura del atractor.

Configuración:
- Parámetros clásicos de Lorenz
- n_components = 3 (x,y,z)
- T = 5000
- Integración con dt = 0.02
- Inyecciones de rho para simular transiciones

Todo el resto del pipeline (RECD, hiper-persistencia, RQA Config B, detección, figuras, CSV)
es idéntico al usado en el mapa logístico.

Salida: resultados_sintetico_lorenz/
"""
...
CONFIG = {
    ...
    # Parámetros clásicos del atractor de Lorenz
    "sigma": 10.0,
    "rho": 28.0,
    "beta": 8.0 / 3.0,
    
    # Inyecciones de régimen (cambio de rho en momentos específicos)
    # Formato: (t_index, new_rho)
    "injections": [
        (1500, 35.0),   # Aumento de rho (más caótico / diferente estructura)
...
OUTPUT_DIR = "resultados_sintetico_lorenz"
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("=" * 82)
print("ANÁLISIS REFINADO + RQA - ATRACTOR DE LORENZ (SINTÉTICO)")
...
print(f"Inyecciones de rho: {CONFIG['injections']}")
print(f"RQA Config B: LAM>={CONFIG['LAM_THRESHOLD']}, TT>={CONFIG['TT_THRESHOLD']}")
print("=" * 82)


# =============================================================================
# GENERADOR DE LORENZ CON INYECCIONES DE RÉGIMEN
# =============================================================================
...
def generate_lorenz(T=None, dt=None, sigma=None, rho=None, beta=None, 
    ...
    """
    Genera la trayectoria del atractor de Lorenz (x, y, z).
    
    Soporta inyecciones de cambios de régimen cambiando el parámetro rho
    en momentos específicos (t_index).
    
    Devuelve:
        X: array (T, 3) con columnas [x, y, z]
        change_times: lista de tiempos de inyección (en pasos de índice)
    """
    ...
    # Manejo de inyecciones (cambios de rho)
    if not injections:
        X = odeint(lorenz_deriv, state0, t, args=(sigma, rho, beta))
        change_times = []
    else:
        # Ordenar inyecciones por tiempo
        ...
        for inj_idx, new_rho in inj_list:
            ...
            if len(t_piece) > 1:
                ...
                pieces.append(sol[:-1])  # evitar duplicado en la unión
                current_state = sol[-1]
        ...
        X = np.vstack(pieces)[:T]  # asegurar longitud exacta
    
    # Asegurar que X tenga exactamente T filas
    if len(X) > T:
        X = X[:T]
    elif len(X) < T:
        # padding si es necesario (raro)
        pad = np.tile(X[-1:], (T - len(X), 1))
        X = np.vstack([X, pad])
    
    print(f"  Trayectoria Lorenz generada: shape={X.shape}")
    return X, change_times


# =============================================================================
# FUNCIONES DEL PIPELINE (REUTILIZADAS DEL SCRIPT ANTERIOR)
# =============================================================================
...
def export_enriched_csv(metrics, structural_peaks, structural_changes, rqa_measures, injected_times, output_dir):
    ...
    out_path = os.path.join(output_dir, "picos_fuertes_refinados_rqa_lorenz.csv")
...
def save_figures(metrics, base_peaks, structural_peaks, struct_changes, dist_smooth, dist_thresh,
    ...
    # Fig 2: I con marcadores diferenciados
    fig, ax = plt.subplots(figsize=(11, 5))
    for i in range(n_reg):
    ...
    ax.set_title("Figura 2: I_i(k) Lorenz (refinado + RQA)\n○ base | ◆ estructural (RQA) | verde = inyecciones de rho")
    ax.legend()
    plt.tight_layout()
    ...
    axes[0].set_title("Evolución RQA sobre τ_s (Lorenz) - Config B")
...
def main():
    # 1. Generar datos Lorenz
    print("\n[1] Generando trayectoria del atractor de Lorenz...")
    ...
    leads_to_inj = compute_leads(struct_k, injected_times)
    ...
    # 4. Resumen
    print("\n" + "=" * 82)
    print("RESUMEN FINAL - ATRACTOR DE LORENZ (REFINADO + RQA Config B)")
    print("=" * 82)
