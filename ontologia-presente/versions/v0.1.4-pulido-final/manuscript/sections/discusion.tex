...
El marco es exploratorio y enfrenta objeciones legítimas que se exponen de forma directa.

\textbf{Riesgo de reificación de las «capas».} Las capas se definen por marcadores operacionales. El marco no las presenta como entidades sustanciales separadas de los procesos de persistencia, sino como una jerarquía analítica débil. La partición categorial deberá revisarse si la discreción observada depende fuertemente del umbralado o de la resolución temporal elegida.

\textbf{Carácter heurístico del término «resistencia».} El concepto operativo central es el filtrado probabilístico (efecto estructural sin agente). Los términos «resistencia ascendente/descendente» se usan solo como etiquetas heurísticas secundarias entre comillas y pueden refinarse o sustituirse por «filtrado probabilístico ascendente/descendente».

\textbf{Ausencia de mecanismo de causación descendente.} El marco describe que una estructura coherente (Capa 3) reduce la probabilidad de impacto de fluctuaciones locales fragmentadas, pero no especifica el mecanismo causal preciso (retroalimentación, restricciones topológicas u otro). Esta limitación exige modelado adicional o experimentos de intervención.

\textbf{Posible circularidad entre métricas y ontología.} Las definiciones descansan en las mismas métricas que detectan los fenómenos. Aunque el marco enfatiza que las capas introducen propiedades nuevas (coherencia de escalas; reorganización global de la matriz), la distinción entre hallazgo métrico y afirmación ontológica requiere mayor elaboración filosófica.

\textbf{Subdeterminación de la ontología por los datos.} Los resultados provienen de sistemas sintéticos controlados y casos reales limitados. Otras ontologías podrían acomodar los mismos patrones. El marco se presenta como una síntesis coherente; su valor dependerá de generar predicciones contrastables y de dialogar con otras tradiciones (en particular con la filosofía de Leonardo Polo). Se reconoce explícitamente la subdeterminación actual.