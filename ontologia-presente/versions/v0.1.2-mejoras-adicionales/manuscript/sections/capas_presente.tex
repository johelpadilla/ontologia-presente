...
\textbf{Estatus ontológico de la Capa 2}. La Capa 2 tiene estatus ontológico pleno dentro de la jerarquía propuesta, pero su naturaleza es primariamente relacional y transicional. A diferencia de la Capa 1 —que localiza una intensificación de densidad y adhesión en módulos individuales— o de la Capa 3 —que introduce una reorganización estructural del sistema como totalidad—, la Capa 2 no añade un «presente» con localización sustantiva o densidad propia equivalente. Su contribución ontológica específica es la emergencia de la coherencia de escalas como propiedad del acoplamiento mismo. En este sentido, funciona como un «puente ontológico» o capa de transición crítica: su rol principal es habilitar (o bloquear, según el nivel de antisincronización) el paso de intensificaciones locales a efectos estructurales globales. Esta caracterización evita tanto reducirla a un mero «estado intermedio» sin peso ontológico como atribuirle la misma forma de «presente» que las otras dos capas.
...
\begin{importantbox}
La transición de la Capa 1 a la Capa 3 no es automática. Una intensificación local que ocurre bajo alta antisincronización (alta fragmentación) tiende a permanecer como un «fragmento ontológico»: la probabilidad de que genere reorganización estructural detectable (Capa 3) se reduce sustancialmente. Este efecto de filtrado probabilístico en dirección ascendente se denomina, entre comillas y con valor heurístico, «resistencia ascendente». Solo cuando la antisincronización desciende —es decir, cuando opera el efecto de filtrado probabilístico en dirección descendente que denominamos, entre comillas, «resistencia descendente»— se crean las condiciones de permeabilidad necesarias para que la intensificación local adquiera peso estructural.
\end{importantbox}

\subsection{Jerarquía y no reductibilidad}

Las tres capas constituyen una jerarquía ontológica estricta: cada capa superior presupone condiciones de la inferior, pero introduce propiedades relacionales o globales nuevas que no son deducibles a partir del análisis de los componentes de la capa inferior tomados aisladamente. A continuación se explicita qué propiedad irreducible añade cada capa superior:

\begin{itemize}
    \item La Capa 2 (coherencia de escalas de persistencia) presupone la existencia de presentes locales (Capa 1), pero añade la propiedad relacional nueva e irreducible de la \textbf{alineación (o coherencia) de escalas de persistencia $\tau_s$} —cuantificada por la reducción de la desviación estándar entre componentes en ventanas rodantes—. Esta propiedad no existe en ningún módulo individual ni puede obtenerse a partir de la colección de los valores de $\tau_s$ de los módulos considerados sin referencia a su dispersión mutua. Es una propiedad del acoplamiento, no de las partes.
    \item La Capa 3 (reorganización estructural) presupone Capas 1 y 2, pero introduce la propiedad global nueva e irreducible de la \textbf{reorganización de la matriz de relaciones inter-modulares}, cuya magnitud de cambio se mide mediante la norma de Frobenius entre matrices de coherencia en ventanas consecutivas. Esta propiedad del sistema como totalidad (la alteración de la geometría relacional global) no puede predecirse ni deducirse del mero listado de intensificaciones locales ni de las escalas de persistencia individuales, incluso cuando se conozca su alineación.
\end{itemize}
