...
\begin{importantbox}
La intensificación local del presente (Capa 1) genera efectos estructurales (Capa 3) con mayor probabilidad cuando los módulos presentan escalas de persistencia más coherentes entre sí (baja antisincronización, Capa 2). De lo contrario, la intensificación permanece como un fragmento aislado que el sistema puede absorber sin reorganizarse. Este efecto de filtrado probabilístico en dirección ascendente se denomina, entre comillas y con valor heurístico secundario, «resistencia ascendente».
\end{importantbox}
...
Cuando se añadió ruido gaussiano controlado (0 %, 5 %, 10 % y 15 % de la desviación estándar de cada componente) al mapa logístico $r=3.8$, el número de picos estructurales descendió de 121 a 100, mientras que el número de cambios estructurales detectados por norma de Frobenius permaneció esencialmente invariante. El detector conserva utilidad práctica hasta niveles de ruido del orden del 10--15 %, con degradación gradual. Este resultado indica que la intensificación del presente posee una estabilidad estructural que trasciende fluctuaciones superficiales.
...
Los hallazgos empíricos no se interpretan aquí como meras corroboraciones numéricas, sino como evidencia de que la \textit{discreción} detectada por el RECD corresponde a transiciones entre distintos niveles de integración de la persistencia. La universalidad de Feigenbaum, la robustez al ruido y —de manera especialmente relevante— el papel regulador de la antisincronización (reducción sistemática y estadísticamente significativa previa a picos estructurales) convergen en la estructura jerárquica que se desarrolla a continuación. Esta evidencia motiva directamente la partición ontológica en tres capas y la caracterización del filtrado probabilístico bidireccional como el mecanismo estructural que modula la probabilidad de tales transiciones, tal como se expone en las Secciones 4 y 5.