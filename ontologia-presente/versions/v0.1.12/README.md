...
**✅ VERSIÓN BASE / CONGELADA v0.1.12 — 2026-06-15**

Esta es la **nueva versión base oficial** del manuscrito (sucesora de v0.1.0-frozen).

Contiene el manuscrito completo y pulido:
- 14 páginas con compilación limpia.
- Tabla de Contenidos (Índice) completa.
- Sección "Referencias" con entradas y DOIs.
- Diagrama conceptual de las tres capas (Figura 4) en su estado preferido y equilibrado (restaurado de la v0.1.10 que renderizó perfectamente, sin desbordamiento de márgenes).
- Todo el pulido ligero de fluidez aplicado en rondas posteriores (Introducción, Implicaciones para la ontología, Jerarquía y no reductibilidad), sin cambios conceptuales.
- Terminología rigurosa: «filtrado probabilístico bidireccional» como mecanismo estructural principal; «resistencia ascendente/descendente» solo como heurístico secundario ocasional entre comillas.

**Política a partir de esta versión:**
- Esta carpeta (`versions/v0.1.12/`) es ahora la **versión base de referencia** para citación, revisiones, envío a revista y registro Zenodo.
- **Nunca editar directamente aquí** una vez congelada.
- Todo trabajo futuro (nuevas revisiones, feedback de revista, ampliaciones, etc.) debe realizarse **exclusivamente** copiando esta versión a una nueva subcarpeta dentro de `versions/` (ej. `versions/v0.1.13-xxx/`).
- La raíz del proyecto (`ontologia-presente/`) y las versiones anteriores son solo para referencia histórica.
