...
### Base (nueva versión base / congelada del manuscrito)

Esta versión se establece como la **nueva versión base oficial** (sucesora de v0.1.0-frozen). Incluye:

- Todo el contenido y pulido acumulado hasta v0.1.11.
- Restauración de la **Figura 4** al estado exacto que renderizó de forma perfecta y equilibrada (sin overflow de margen derecho) en v0.1.10-pulido-final, según preferencia del autor. El diagrama mantiene buen espaciado vertical, legibilidad y encaja limpiamente dentro de los márgenes de 2.5 cm.
- Actualización de metadatos y nombres de archivos para reflejar que esta es la versión base limpia a utilizar de ahora en adelante.

### Fixed (histórico de la corrección de Figura 4)
- **Corrección de la Figura 4**: Ajuste del diagrama conceptual TikZ para que no se salga del margen derecho y quede completamente dentro de los márgenes del documento (geometry 2.5cm). Se introdujo `scale=0.9` en el entorno `tikzpicture`; se redujo el ancho total de las cajas de las tres capas (minimum width=7.8cm, text width=7.5cm, inner sep=5pt); se acortaron las protrusiones horizontales de las flechas (xshift=0.65cm y ++(0.8,0) en lugar de valores mayores); se reorganizó ligeramente la disposición de las etiquetas laterales (filtrado ascendente/descendente, requiere ↓A(k), condición necesaria) usando posicionamiento `above` en lugar de `right`/`left` + yshift para que no extiendan el ancho horizontal; se acercó la anotación lateral de antisincronización (de x=-6.2 a -5.5); se ajustaron las coordenadas y de nodos (título, cajas y etiqueta) para compensar el escalado y mantener el espaciado vertical aireado y la legibilidad. Cambios idénticos aplicados al código TikZ en `capas_presente.tex` (dentro del entorno figure) y en `figures/layers_diagram.tex`.
- Estructura jerárquica, colores diferenciados, flechas de integración (sólidas) y filtrado (discontinuas), y todo el contenido del diagrama se conservan exactamente.
- Restricciones estrictas respetadas: **solo** se modificó la Figura 4 y su código TikZ; ningún cambio de texto, conceptual, en otras secciones, otras figuras ni en el resto del manuscrito.
- Compilación final limpia (secuencia completa de 4 pases: limpieza de auxiliares + pdflatex + bibtex + pdflatex + pdflatex) dentro de `manuscript/`. El resto del documento (14 páginas, TOC, Referencias) permanece idéntico a v0.1.11. CHANGELOG actualizado dentro de la carpeta de versión.

**Actualización posterior (preferencia del autor)**: 
- **Figura 4 restaurada al estado de v0.1.10-pulido-final**: El usuario indicó que la Figura 4 compilada en `versions/v0.1.10-pulido-final` salió perfecta (buen equilibrio, espaciado, legibilidad y sin desbordamiento de margen derecho). Se reemplazó el código TikZ completo de la Figura 4 (en `sections/capas_presente.tex` y `figures/layers_diagram.tex`) por el exacto de la v0.1.10: `node distance=0.9cm`, cajas `minimum width=8.0cm / text width=7.7cm / inner sep=5pt / min height=1.2cm`, posiciones y (título 5.1, c3 3.9, c2 2.2, c1 0.5), extensiones de flechas `xshift=0.7cm / ++(1.1,0)`, etiquetas laterales con `right`/`left` + yshifts originales, y anotación de antisincronización en `(-5.6, 2.0)`. Se recompiló con la secuencia completa de 4 pases. El diagrama ahora coincide visualmente con el que el autor considera perfecto en la v0.1.10 (sin overflow, proporciones equilibradas). Solo se tocó el código del diagrama; el texto circundante y el resto del manuscrito conservan los avances de pulido de v0.1.11/v0.1.12. Verificación: PDF de 14 páginas sin warnings de márgenes/overfull en la figura, etiquetas del diagrama presentes en la capa de texto.
