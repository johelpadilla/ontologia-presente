...
**Note on recent history (v0.4 onward):** Detailed technical and framing changes for v0.4–v0.7 are recorded inside each snapshot's own `versions/vX-.../CHANGELOG.md`. The root CHANGELOG preserves the early foundational entries.

## [2026-06-18] GitHub preparation & repository hygiene

- Created `.gitignore` (local to `joint-episode-paper/`) with explicit un-ignore rules for the named draft PDFs (`Joint-Episode-Paper-Draft-*.pdf`) and key reproducibility artifacts (`code/results/*.json`, `*.csv`). Parent-level ignores for build products and generated figures are respected.
- Removed stale build artifacts (`manuscript/main.pdf`, all `__pycache__/`, `*.aux`/ `*.log` etc. across versions and root).
- Rewrote `README.md` with current status (v0.7), honest scientific framing (L1–L2 primacy + AUC ≈ 0.476 oracle ceiling), accurate project structure, build instructions, and strict versioning policy.
- Verified via dry-run that sources, version snapshots (including their `main.pdf`), named root PDFs, and data summaries are addable.

These files are now ready for staging and push. No manuscript content changes were made in this step.
