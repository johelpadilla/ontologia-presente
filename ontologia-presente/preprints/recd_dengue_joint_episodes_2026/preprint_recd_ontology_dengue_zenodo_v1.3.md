...
- Fixed all figure captions to eliminate duplications (e.g., "Figure X: Figure X") and rewrote them to be precise, self-contained, and to include explicit ontological interpretations.  
- Enforced consistent logical numbering throughout: Figure 1 (Three-Layer Framework), Figure 2 (regime contrast), Figure 3 (calibration diagram), Figure 4 (Alto performance); updated all cross-references and separated combined figures in LaTeX.  
- Improved LaTeX typography for a clean professional PDF: refined geometry, figure floats and spacing, URL breaking, and eliminated margin overflows without excessive use of `\sloppy`.  
- Strengthened the justification for synthetic data in Phase 8, emphasizing its positive epistemic necessity for isolating Layer 1–Layer 2 dynamics under controlled ontological conditions.  
- Deepened the Discussion of Iquitos as a qualitatively distinct dynamical regime, clarifying implications for the three-layer framework and operational deployment.  
- Made the connection between the ontological premise and the four-component construction of the Confidence Score explicit and conceptually coherent.  
- Expanded Limitations into a dedicated section with precise, self-critical points on data, model, and generalizability constraints.  
- Performed comprehensive language review: shortened long sentences, improved paragraph flow and transitions, and enhanced precision while retaining formal academic tone.  
- Updated Version History in both .md and .tex files.
...
**Long title:** From recurrence to reorganization: A three-layer ontological framework and operational early-warning system for dengue using Joint episodes (Layer 1 + Layer 2) with synthetic validation for data-scarce regimes
...
![Figure 1. The Three-Layer Ontological Framework and the Joint Episode. Layer 1 (local intensification via high recurrence M and trapping time) occurs within a sustained Layer 2 window of low antisynchrony (permeability). Their joint realization for adequate duration and intensity defines the Joint episode—the primary analytical and operational unit. Only this configuration raises the probability of transition to Layer 3 (structural reorganization/outbreak).](./figures/fig2_three_layer_framework.png)

**Ontological interpretation:** Layer 1 intensification alone remains local and insufficient. Sustained Layer 2 provides the permeable medium allowing local changes to propagate and scale. The Joint episode is thus the detectable precursor state whose intensity (Joint Score) and persistence the early-warning system quantifies.
...
Real-world data supply too few observed Joint episodes for stable weight selection or probability calibration and entangle site-specific covariates with the very parameters of interest. From an ontological standpoint, synthetic generation is not a workaround but an epistemically necessary instrument: the three-layer framework posits that the probability of Layer 3 reorganization is a function of the integrated intensity and persistence of Layer 1 within Layer 2. To test this claim rigorously, one must independently vary Layer 1 intensity (mean\_M, consistency) and Layer 2 duration while holding the mapping to latent transition probability fixed. Only controlled synthetic episodes afford such orthogonal manipulation, allowing isolation of layer contributions and direct adjudication of whether observed site differences represent quantitative shifts in the same dynamical regime or qualitatively distinct layer interactions.
...
![Figure 2. Ontological contrast between dynamical regimes (synthetic data). Boxplots of Joint Score (log scale, left) and episode duration (right) for synthetic Joint episodes, stratified by regime (San Juan-like vs. Iquitos-like) and by transition to Layer 3. San Juan-like episodes are substantially longer and higher-intensity, with clear separation by outcome. Iquitos-like episodes are short and weak, showing minimal differentiation by outcome—consistent with intrinsically rare sustained Layer 2 states.](./figures/fig1_regime_contrast.png)

**Interpretation:** The synthetic generator isolates the ontological distinction. In the San Juan-like regime, integrated Layer 1–Layer 2 intensity and persistence strongly covary with Layer 3 transition. In the Iquitos-like regime the bulk of episodes lie at low values with negligible separation. This indicates that Iquitos is not merely data-poor San Juan but a qualitatively different dynamical regime in which the conditions for Layer 2 persistence are structurally constrained.
...
![Figure 3. Calibration (reliability) diagram for the San Juan-like regime. Observed frequency of Layer 3 transition plotted against mean predicted Confidence Score in bins, using the recommended weight vector. Proximity of points to the diagonal indicates calibration quality; modest bias is visible but recoverable.](./figures/reliability_san_juan_like.png)

**Ontological interpretation:** The diagram shows that the four-component Confidence Score (probability of C3, normalized Joint Score, normalized duration, and Layer 1 consistency) recovers the latent transition structure generated from the three-layer premise. Good calibration supports treating the score as graded evidence of maturation toward reorganization rather than a purely statistical artifact.

Reliability diagrams (Figure 3) indicated that the synthetic ground-truth transition probabilities were recoverable with modest bias, supporting future isotonic recalibration once larger real samples become available.

![Figure 4. Performance of the “Alto” category under the recommended weight vector in the San Juan-like regime. The categorization concentrates episodes with elevated empirical Layer 3 transition rates. In contrast, the Iquitos-like regime produced virtually no “Alto” episodes, revealing regime-dependent expression of the Layer 1 + Layer 2 configuration.](./figures/category_perf_san_juan_like.png)

**Ontological interpretation:** Categorical prioritization is not regime-invariant. The “Alto” threshold isolates high-intensity, high-persistence Joint episodes only where sustained Layer 2 is sufficiently prevalent; in low-permeability regimes such as Iquitos-like conditions, the ontology predicts—and the data confirm—the absence of high-confidence precursors.

### Real-Data Context and Operational Implications

The 11 Joint episodes recovered for San Juan and three for Iquitos yielded apparently high success rates but remain too few for stable parameterization. Episode-level alerts with categorical output reduce notification volume by more than an order of magnitude relative to per-week models while preserving coverage of Layer 3 events under high-prevalence conditions. The “Alto” category concentrates those episodes most likely to warrant prioritized response.

---

## Discussion

The synthetic results demonstrate that the three-layer framework does more than organize recurrence features; it exposes genuine heterogeneity in the interaction of Layer 1 and Layer 2 across settings. The central finding is that the near-absence of high-confidence Joint episodes under Iquitos-like parameters is not an artifact of small sample size. It indicates that sustained Layer 2 states—periods of low antisynchrony long enough to contain Layer 1 intensification—are intrinsically rarer or shorter in that environment.

This difference carries direct ontological implications. The framework treats Layer 2 (permeability) as the necessary medium that allows local Layer 1 intensification to scale to Layer 3 reorganization. If the statistical properties of Layer 2 persistence differ systematically between locations, then the quantitative mapping from layer parameters to transition probability is itself regime-dependent. Iquitos is therefore not a scaled-down or data-poor version of San Juan; it instantiates a distinct dynamical regime in which the conditions enabling prolonged permeability are structurally constrained, whether by vector ecology, urban topology, climatic forcing, or human mobility. The three-layer ontology remains valid, yet its observable signatures and the intensity thresholds required for reorganization are not universal.

For operational deployment the distinction is consequential. A single set of weights or category thresholds calibrated on high-prevalence regimes such as San Juan-like conditions will systematically under-flag (or fail to flag) episodes in Iquitos-like settings. Site-specific recalibration of the Confidence Score, or the adoption of auxiliary indicators of Layer 2 duration, becomes necessary. Episode-level alerting still offers dramatic reduction in notification burden relative to per-week models, but the “Alto” decision surface must be allowed to vary with the local expression of the permeability layer. In short, the ontology demands regime-aware parameterization rather than naive transfer of alert rules.

The Confidence Score’s empirical sensitivity to duration and Joint Score under high-prevalence conditions further validates the decision to anchor its construction in the ontology. These two components directly quantify the persistence and integrated force of the Layer 1 + Layer 2 configuration; the remaining terms (model probability and Layer 1 consistency) supply complementary evidence of local intensification and predictive support. The synthetic experiments thereby convert a methodological necessity into an empirical demonstration that the four-component structure is not arbitrary but follows from the causal ordering posited by the three-layer framework.

## Limitations

Several limitations must be acknowledged. First, the synthetic generator relies on regime-specific normal distributions and a logistic link estimated from limited real episodes; it does not yet incorporate time-varying covariates such as vector-control interventions, rainfall anomalies, or serotype circulation that modulate real transition probabilities. Second, the current evaluation uses only two sites and three synthetic regimes; broader generalizability across ecological and urban contexts remains to be tested. Third, the Confidence Score weights were selected from a small discrete grid rather than optimized continuously; a larger real sample will permit more refined isotonic or beta calibration. Fourth, episode detection itself depends on the choice of antisynchrony and M thresholds; sensitivity analyses performed in earlier phases support robustness, yet fully data-driven threshold learning was outside the present scope. Finally, prospective operational validation with documented response outcomes has not yet been conducted. All of these constraints will be addressed in Phase 9 through extended Puerto Rico datasets and field pilot deployment.
...
*Preprint deposited on Zenodo – June 2026. Version 1.3. Author: Dr. Johel Padilla Villaunueva. This document consolidates work from Phases 1–8 of the RECD + Ontología del Presente (Tau Sistémico) project.*