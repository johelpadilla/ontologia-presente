...
- Fixed figure numbering once and for all. Eliminated the inconsistency in which text around page 3 (or in references to the early diagram) referred to the ontological framework as “Figure 2” while its caption stated “Figure 1”. Enforced the definitive numbering throughout the entire document in both .md and .tex.  
- Established final numbering with explicit verification: Figure 1 (The Three-Layer Ontological Framework and the Joint Episode), Figure 2 (Ontological contrast between dynamical regimes (synthetic data)), Figure 3 (Calibration (reliability) diagram for the San Juan-like regime), Figure 4 (Performance of the “Alto” category under the recommended weight vector).  
- Corrected every figure reference: all prose mentions, image alt text, visible captions in MD, LaTeX \caption texts, \label and \ref commands, and figure placement order in the source. Added explicit in-text references at the point of presentation for each figure (e.g., “illustrated in Figure 1”, “shown in Figure 2”).  
- Added visible **Figure X.** captions directly in the Markdown body (below each image) so numbering and full captions appear explicitly in rendered MD, matching the LaTeX output.  
- Rewrote all four figure captions to be clear, professional, concise, and elegant while embedding a short ontological interpretation in each. Removed redundant phrasing.  
- Refined the 1–2 bridging sentences between the end of the Discussion and the Limitations section for smoother, more natural logical flow.  
- Performed a final language polish: shortened several remaining long or dense sentences (especially in Discussion and Limitations), tightened paragraph transitions, and improved overall clarity and precision while preserving formal tone and scientific content.  
- Updated all version strings, headers, footers, PDF metadata, and the Version History itself to 1.5.  
- Verified that the LaTeX source compiles cleanly to a professional PDF with no margin overflows, correct sequential figure placement, and proper spacing.
