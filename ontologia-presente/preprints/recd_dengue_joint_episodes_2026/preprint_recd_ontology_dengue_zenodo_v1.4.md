...
- Established strict final figure numbering across the document—Figure 1 (The Three-Layer Ontological Framework and the Joint Episode), Figure 2 (Ontological contrast between dynamical regimes), Figure 3 (Calibration diagram), Figure 4 (Performance of the “Alto” category)—and aligned every text reference, image alternative text, caption, and LaTeX label with zero mismatches.  
- Polished all four figure captions to be concise, precise, and elegant while retaining essential ontological interpretations; removed redundant phrasing in both .md and .tex.  
- Added one to two bridging sentences at the close of the Discussion to create a smoother, more natural transition into the Limitations section.  
- Conducted a final language review of the entire manuscript: shortened remaining long or dense sentences (particularly in Discussion and Limitations), improved paragraph-to-paragraph flow, and increased clarity and precision without altering scientific meaning.  
- Refined the LaTeX source for clean, professional compilation: updated all version identifiers, tuned figure placement and spacing parameters, applied targeted line-breaking controls for long elements, and eliminated margin overflows.  
- Preserved the existing References unchanged while ensuring the explicit note regarding future expansion after external validation is clear and visible in both file formats.  
- Updated Version History, headers, footers, and metadata from 1.3 to 1.4 in both .md and .tex.
...
![Figure 2. Ontological contrast between dynamical regimes (synthetic data). Boxplots of Joint Score (log scale) and episode duration for synthetic Joint episodes, stratified by regime and by Layer 3 outcome. San Juan-like episodes are substantially longer and higher-intensity, with clear separation by outcome. Iquitos-like episodes are short and weak, showing minimal differentiation—consistent with intrinsically rare sustained Layer 2 states.](./figures/fig1_regime_contrast.png)

**Ontological interpretation:** The synthetic generator isolates the distinction between regimes. In the San Juan-like regime, integrated Layer 1–Layer 2 intensity and persistence strongly covary with Layer 3 transition. In the Iquitos-like regime most episodes lie at low values with negligible separation. This indicates that Iquitos is not merely data-poor San Juan but a qualitatively different dynamical regime in which sustained Layer 2 states are structurally constrained.
...
![Figure 4. Performance of the “Alto” category under the recommended weight vector in the San Juan-like regime. The high-confidence category concentrates episodes with elevated empirical Layer 3 transition rates. In the Iquitos-like regime virtually no episodes reached the “Alto” threshold, revealing regime-dependent expression of Layer 1 + Layer 2 conditions.](./figures/category_perf_san_juan_like.png)
...
The Confidence Score’s empirical sensitivity to duration and Joint Score under high-prevalence conditions further validates anchoring its construction in the ontology. These two components directly quantify the persistence and integrated force of the Layer 1 + Layer 2 configuration. The remaining terms supply complementary evidence of local intensification and predictive support. The synthetic experiments thereby convert a methodological necessity into an empirical demonstration that the four-component structure follows from the causal ordering posited by the three-layer framework.

These findings reinforce the explanatory value of the three-layer ontology and the operational advantages of episode-level alerts. At the same time, they highlight constraints that limit immediate claims to generalizability and underscore the priorities for external validation.

## Limitations

Several limitations must be acknowledged. The synthetic generator relies on regime-specific normal distributions and a logistic link estimated from limited real episodes. It does not yet incorporate time-varying covariates such as vector-control interventions, rainfall anomalies, or serotype circulation. The current evaluation uses only two sites and three synthetic regimes; broader generalizability across ecological and urban contexts remains to be tested. The Confidence Score weights were selected from a small discrete grid rather than optimized continuously; a larger real sample will permit more refined calibration. Episode detection depends on the choice of antisynchrony and M thresholds. Sensitivity analyses support robustness, yet fully data-driven threshold learning lay outside the present scope. Prospective operational validation with documented response outcomes has not yet been conducted. All of these constraints will be addressed in Phase 9 through extended Puerto Rico datasets and field pilot deployment.
...
*Preprint deposited on Zenodo – June 2026. Version 1.4. Author: Dr. Johel Padilla Villaunueva. This document consolidates work from Phases 1–8 of the RECD + Ontología del Presente (Tau Sistémico) project.*