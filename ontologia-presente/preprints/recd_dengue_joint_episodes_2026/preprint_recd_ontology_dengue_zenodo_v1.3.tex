% =============================================================================
% preprint_recd_ontology_dengue_zenodo_v1.3.tex
...
\hypersetup{
    ...
    pdftitle={RECD + Three-Layer Ontology for Early Warning of Dengue Outbreaks (v1.3)},
...
\fancyhead[R]{\footnotesize Preprint v1.3 -- Zenodo}
...
\date{17 June 2026 \\ \small Version 1.3 — Preprint for Zenodo}
...
Real-world data supply too few observed Joint episodes for stable weight selection or probability calibration and entangle site-specific covariates with the parameters of interest. From an ontological standpoint, synthetic generation is not merely a workaround for scarcity: the three-layer framework posits that the probability of Layer 3 reorganization is a function of the integrated intensity and persistence of Layer 1 within Layer 2. To test this claim rigorously one must vary Layer 1 intensity (mean\_M, consistency) and Layer 2 duration independently while holding the mapping to latent transition probability fixed. Only controlled synthetic episodes afford such orthogonal manipulation, enabling isolation of each layer's contribution and adjudication of whether site differences are quantitative or reflect qualitatively distinct layer dynamics.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig1_regime_contrast.png}
...
\caption{Calibration (reliability) diagram for the San Juan-like regime under the recommended weight vector. Observed frequency of Layer 3 transition is plotted against binned predicted Confidence Score. Points close to the diagonal indicate good calibration, with modest recoverable bias.}
\label{fig:calibration}
\end{figure}

Reliability diagrams (Figure~\ref{fig:calibration}) indicated that the synthetic ground-truth transition probabilities were recoverable with modest bias, supporting future isotonic recalibration once larger real samples become available.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{figures/category_perf_san_juan_like.png}
\caption{Performance of the ``Alto'' category under the recommended weight vector in the San Juan-like regime. The categorization concentrates episodes with elevated empirical transition rates. The Iquitos-like regime produced virtually no ``Alto'' episodes, confirming that high-confidence Joint episodes are regime-dependent.}
\label{fig:alto}
\end{figure}

The concentration of actionable episodes in the ``Alto'' category is shown in Figure~\ref{fig:alto}.
...
\vspace{0.6em}
\noindent\textit{Preprint deposited on Zenodo – June 2026. Version 1.3.}  
