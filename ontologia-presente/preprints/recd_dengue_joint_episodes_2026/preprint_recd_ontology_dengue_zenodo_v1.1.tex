% =============================================================================
% preprint_recd_ontology_dengue_zenodo_v1.1.tex
...
\hypersetup{
    ...
    pdftitle={RECD + Three-Layer Ontology for Early Warning of Dengue Outbreaks (v1.1)},
...
\fancyhead[R]{\footnotesize Preprint v1.1 -- Zenodo}
...
\date{17 June 2026 \\ \small Version 1.1 — Preprint for Zenodo}
...
The principal limitation addressed here is the scarcity of observed Joint episodes in empirical data, particularly for Iquitos. To overcome this constraint while preserving ontological fidelity, we constructed a synthetic generator that produces controlled realizations of Capa 1 + Capa 2 configurations under distinct dynamical regimes. This enables systematic calibration and reveals regime-specific behavior that real data alone cannot reliably characterize.

We deposit this version to support reproducibility and to solicit scholarly feedback prior to external validation with extended Puerto Rico data. Comments may be submitted via Zenodo or the associated repository.
\end{zenodolletter}
...
\caption{The Three-Layer Ontological Framework and the Joint Episode. Capa 1 (intensification) occurring inside a sustained window of Capa 2 (low antisynchrony/permeability) constitutes the Joint episode — the fundamental analytical and operational unit. The framework posits that only when both layers co-occur for sufficient duration and intensity does the probability of transition to Capa 3 (reorganization) become appreciable.}
\label{fig:framework}
\end{figure}
...
In the Iquitos-like regime, no weighting produced any “Alto” classifications (Alto F1 = 0). Mean confidence across episodes remained low (0.214--0.258). The noisy balanced regime produced intermediate results consistent with the ranking observed under high-prevalence conditions.
...
\caption{Left: Calibration (reliability) diagram for the San Juan-like regime. Right: Performance of the “Alto” category. The categorization concentrates actionable episodes. In the Iquitos-like regime almost no episodes reached the “Alto” threshold.}
\label{fig:calib_perf}
\end{figure}

\subsection{Real-Data Context and Operational Implications}

The 11 Joint episodes recovered for San Juan and three for Iquitos yielded apparently high success rates but are too few for stable parameterization. Episode-level alerts with categorical output reduce notification volume by more than an order of magnitude relative to per-week models while preserving coverage of Capa 3 events in high-prevalence conditions.
...
The Confidence Score’s sensitivity to duration and Joint Score under high-prevalence conditions further illustrates the value of anchoring weights in the ontology. The framework thereby converts a methodological necessity (synthetic data) into an empirical discovery (regime heterogeneity) with direct implications for site-specific operational design.
...
\begin{enumerate}
    ...
    \item For Iquitos-like contexts, consider modestly lower “Alto” thresholds or auxiliary indicators of Capa 2 persistence.
...
\textbf{\url{https://github.com/johelpadilla/ontologia-presente}} (subfolder: \texttt{preprints/recd\_dengue\_joint\_episodes\_2026})
...
\hrule
\vspace{0.5em}
\noindent\textit{Supplementary Material:} Code, synthetic datasets, figures, the external validation protocol, and detailed planning documents are provided in the repository subfolder:

\vspace{0.2em}
\url{https://github.com/johelpadilla/ontologia-presente (subfolder: preprints/recd_dengue_joint_episodes_2026)}

\vspace{0.6em}
\noindent\textit{Preprint deposited on Zenodo – June 2026. Version 1.1.}  
