...
- Verified and corrected DOIs/Zenodo persistent identifiers for all references authored by Johel Padilla-Villanueva. Updated entry 5 (padilla2026tau / El Paradigma del Tau Sistémico) to include the declared identifier 10.5281/zenodo.19654897 from the Síntesis Magna source. Confirmed entry 6 (padilla2026permeability) as 10.5281/zenodo.20723966 (declared in the technical report). Entry 4 (tau2026 project reports) retained as placeholder.  
- Reformatted bibliography entries for consistency and visibility; DOIs now appear as clean, clickable https://doi.org/ links in both Markdown and (via new \doi helper) in the compiled LaTeX PDF.  
- Critically assessed the full reference list for sufficiency in supporting the paper’s primary claims (mathematical characterization of the three ontological layers via RQA and Joint episodes; RQA foundations; EWS context; Tau Sistémico/RECD paradigm). Marwan (2007) and Scheffer et al. (2009) provide adequate grounding for methods; the two Padilla-Villanueva 2026 works (now with persistent identifiers) supply the direct paradigm and permeability-validation foundation. No additional references were added; the list is sufficient, focused, and lean.  
- Refined the tone and precision of the ontological-mathematical claim in the Discussion (and lightly in Abstract/Letter): strengthened rigor and clarity while qualifying scope to the controlled synthetic validation and observed results, avoiding overstatement. Improved sentence flow and paragraph transitions around these central claims.  
- Minor language polish and readability improvements in Results and Discussion sections.  
- Updated all version identifiers, headers/footers (in .tex), PDF metadata, and the Version History block to v1.8.  
- Confirmed clean two-pass pdflatex compilation and visible, correctly formatted DOIs in the bibliography.
...
*Preprint deposited on Zenodo – June 2026. Version 1.8. Author: Dr. Johel Padilla Villaunueva. This document consolidates work from Phases 1–8 of the RECD + Ontología del Presente (Tau Sistémico) project.*