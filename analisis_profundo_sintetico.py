#!/usr/bin/env python3
"""
analisis_profundo_sintetico.py

Análisis profundo de los picos estructurales (refinado + RQA) generados
en el experimento sintético con mapa logístico acoplado.
...
- Comparación de perfiles RQA con tests Mann-Whitney
- Análisis de concentración temporal antes de las inyecciones
- Figuras y reporte estructurado

Uso:
    python analisis_profundo_sintetico.py
"""
...
# CONFIGURACIÓN
# =============================================================================
CSV_PATH = "resultados_sintetico/picos_fuertes_refinados_rqa_sintetico.csv"
OUTPUT_DIR = "resultados_sintetico/analisis_profundo"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Transiciones inyectadas conocidas (del generador sintético)
INJECTED_CHANGES = [380, 920]
...
print("=" * 82)
print("ANÁLISIS PROFUNDO DE PICOS ESTRUCTURALES - EXPERIMENTO SINTÉTICO")
print("Config B (Equilibrio) en Mapa Logístico Acoplado")
print("=" * 82)
...
def load_and_prepare(csv_path):
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            ...
            "Ejecuta primero analisis_refinado_rqa_sintetico.py"
        )
...
def generate_figures(df, good, weak, p60, injected_times, output_dir):
    ...
    for i, metric in enumerate(metrics_to_plot):
        sns.boxplot(
            ...
            y=metric, 
            ax=axes[i],
            palette={"Buenos anticipadores": "#2ca02c", "Anticipadores débiles": "#d62728"}
        )
        sns.stripplot(
            data=plot_df, 
            x="anticipator_group", 
    ...
    plt.suptitle("Comparación de métricas RQA: Buenos anticipadores vs Débiles", y=1.02, fontsize=14)
    plt.tight_layout()
    ...
    for inj in injected_times:
        ax.axvline(inj, color="green", linestyle="--", linewidth=2, alpha=0.8, 
                   label="Transición inyectada" if inj == injected_times[0] else "")
    ...
    ax.set_title("Timeline de picos estructurales y transiciones de régimen inyectadas")
    ...
    plt.suptitle("Relación entre lead a inyección y métricas RQA", y=1.02, fontsize=14)
    plt.tight_layout()
...
def generate_report(df, good, weak, rqa_comparison, conc_df, total_before, pct_any, p60, output_dir):
    """Genera el reporte estructurado en consola y archivo."""
    report_lines = []
    ...
    log("=" * 82)
    log("REPORTE DE ANÁLISIS PROFUNDO - PICOS ESTRUCTURALES SINTÉTICOS")
    log(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    log("Sistema: Mapa Logístico Acoplado (4 componentes)")
    ...
    log("   - El marco RECD + RQA (con énfasis en laminaridad y aceleración de trapping) "
        "muestra capacidad de anticipar cambios de régimen en el sistema sintético acoplado.")
    ...
    # Guardar reporte
    report_path = os.path.join(output_dir, "reporte_analisis_profundo_sintetico.txt")
